    ...
    for (int i = 0; i < K; i++)
    {
       ...
       // double val = d[i];
        ...
        for (int j = 0; j < n; j+=4)
        {
            ...
            count1 = count1 + U[i * n + j] * h0[j];
            count2 = count2 + U[i * n + j+1] * h0[j+1];
            count3 = count3 + U[i * n + j+2] * h0[j+2];
            count4 = count4 + U[i * n + j+3] * h0[j+3];
          }
       
        double val = d[i]+count1+count2+count3+count4;
