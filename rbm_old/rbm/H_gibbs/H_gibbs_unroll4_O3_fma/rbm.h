...
void gibbs_H(int * h0, int y0, double *x0);
                         ...
                         double * U, double * W, int n, int D, int K) //DONE
{
    ...
    double* x_for_h = (double*)malloc(sizeof(double)*784);

    for (int i = 0; i < D; i++)
        x_for_h[i] = (double) x0[i];

    myInt64 start;
    start = start_tsc();
#endif
    gibbs_H(h0, y0, x_for_h);
#ifdef PERF_GIBBS_H
    myInt64 cycles = stop_tsc(start);
    count = count + 1;
    delta = cycles - mean;
...
void gibbs_H(int * h0, int y0, double *x0)
{
    ...
    for (int j = 0; j < n; j++)
    {
        int jD = j*D;
        c1 = _mm_load_sd(c+j);
        U1 = _mm_load_sd(U+j*K + y0);
        __m128d sumx = _mm_add_sd(c1, U1);
        _mm_store_sd(&sum, sumx);
        __m128d sum1 = {0,0,0,0};
        __m128d sum2 = {0,0,0,0};
        __m128d sum3 = {0,0,0,0};
        __m128d sum4 = {0,0,0,0};

        for (int i = 0; i < D; i+=4)
        {
            w1 = _mm_load_sd(W + jD + i);
            w2 = _mm_load_sd(W + jD + i+1);
            w3 = _mm_load_sd(W + jD + i+2);
            w4 = _mm_load_sd(W + jD + i+3);

            x1 = _mm_load_sd(x0 + i);
            x2 = _mm_load_sd(x0 + i+1);
            x3 = _mm_load_sd(x0 + i+2);
            x4 = _mm_load_sd(x0 + i+3);

            sum1 = _mm_fmadd_sd(w1, x1, sum1);
            sum2 = _mm_fmadd_sd(w2, x2, sum2);
            sum3 = _mm_fmadd_sd(w3, x3, sum3);
            sum4 = _mm_fmadd_sd(w4, x4, sum4);
        }

        sum1 = _mm_add_sd(sum1, sum2);
        sum1 = _mm_add_sd(sum1, sum3);
        sum1 = _mm_add_sd(sum1, sum4);
