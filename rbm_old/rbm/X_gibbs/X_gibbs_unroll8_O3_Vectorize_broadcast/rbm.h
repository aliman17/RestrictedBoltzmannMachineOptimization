...
void gibbs_X_(int * x, double * h0, double * W, int D, int n);
                         ...
                         double * U, double * W, int n, int D, int K) //DONE
{
    ...
    double* h0d = (double*)malloc(sizeof(double) * n);
    for(int i = 0; i < n; i++)
        h0d[i] = (double)h0[i];
...
void gibbs_X_(int * x, double * h0, double * W, int D, int n)
{
    ...
    for (int i = 0; i < D; i+=8)
    {
        ...
        {
            jD = j * D;
            __m256d h0tmpv = _mm256_broadcast_sd(h0+j);
