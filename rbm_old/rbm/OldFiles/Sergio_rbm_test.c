...
void save_parameters ()
{
    ...
    FILE *writeW;
    writeW = fopen("./Wparam.bin","wb");
    fwrite(W,sizeof(double),n*D,writeW);
    fclose(writeW);

    FILE *writeb;
    writeb = fopen("./bparam.bin","wb");
    fwrite(b,sizeof(double),D,writeb);
    fclose(writeb);

    FILE *writec;
    writec = fopen("./cparam.bin","wb");
    fwrite(c,sizeof(double),n,writec);
    fclose(writec);

    FILE *writed;
    writed = fopen("./dparam.bin","wb");
    fwrite(d,sizeof(double),K,writed);
    fclose(writed);

    FILE *writeU;
    writeU = fopen("./Uparam.bin","wb");
...
int main()
{
    ...
    check_images_labels(320, 222); //check image train and test arrays
    test_get_img_func(232); // check get image and get label functions
