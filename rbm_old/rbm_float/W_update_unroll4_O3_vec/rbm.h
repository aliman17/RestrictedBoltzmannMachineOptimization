...
void gibbs_H(int * h0, int y0, float * x0);
int gibbs_Y_(int* h0, double * U, double * d, int K, int n);
void gibbs_X_(float * x, int * h0, double * W, int D, int n);
...
void image_pp() {
    ...
    images_train = (float *)malloc(sizeof(float) * D * num_img_train);
    images_test = (float *)malloc(sizeof(float) * D * num_img_test);
    
    for (int i = 0; i < num_img_train * D; i++)
    images_train[i] =  (float)images_train_init[i] < 127 ? 0 : 1;
    
    
    for (int i = 0; i < num_img_test * D; i++)
    images_test[i] =  (float)images_test_init[i] < 127 ? 0 : 1;
 ...
 */
float * get_image(int num, float *start)
{
    return start + num * D;
}
 ...
 * h <- sigmoid( c + Wx + Uy )
 */
void h_update(double * h, int y0, float * x0, double * c,
...
void W_update(double * W, double * h0_cap, double * h1_cap,
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < D; j+=4)
        {
            w0 = _mm256_loadu_pd(W);
            w1 = _mm256_loadu_pd(W+1);
            w2 = _mm256_loadu_pd(W+2);
            w3 = _mm256_loadu_pd(W+3);
...
void b_update(double * b, float * x0, float * x1, double lambda, int D){
    for (int i = 0; i < D; i++)
    {
 ...
 */
void COD_training_update(int yi, float * xi, double * h0_cap,
                         ...
                         double * U, double * W, int n, int D, int K) //DONE
{
    ...
    float * x1 = (float *) malloc(sizeof(float) * D);
...
double energy(int y, float * x, double * h, double * W,
