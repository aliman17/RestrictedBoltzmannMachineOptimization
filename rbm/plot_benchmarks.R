...
plot_perf <- function(name, n_koef, n_intercept){
    ...
    perf_proc <- perf/peak_performance * 100  # Get performance percentage
...
# Open config file to read which function to plot and to get the parameters 
# like flops, memory byte transfer
config <- read.table("config") 

for(i in 1:nrow(config)){
  name <- as.character(config[i, 1])
  # We enable line commenting in the config file
