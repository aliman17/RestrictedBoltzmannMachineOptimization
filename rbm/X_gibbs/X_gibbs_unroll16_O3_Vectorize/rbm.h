...
void gibbs_X_(int * x, int * h0, double * W, int D, int n)
{
    ...
    __m256d sum1, sum2, sum3, sum4, w1, w2, w3,w4;
    double rand_n1, rand_n2, rand_n3, rand_n4, rand_n5, rand_n6, rand_n7, rand_n8;
    double h0tmp;
    int jD, iD;
    double arrsum1[4], arrsum2[4], arrsum3[4], arrsum4[4];
    int i, k;
    for (i = 0; i+16 < D; i+=16)
    {
        iD = i*D;
        ...
        sum3 = _mm256_loadu_pd(b+8);
        sum4 = _mm256_loadu_pd(b+12);
        ...
        for (int j = 0; j < n ; j++)
        {
            ...
            w3 = _mm256_loadu_pd(W+jD + i + 8);
            w4 = _mm256_loadu_pd(W+jD + i + 12);

            sum1 = _mm256_fmadd_pd(w1, h0tmpv, sum1);
            sum2 = _mm256_fmadd_pd(w2, h0tmpv, sum2);
            sum3 = _mm256_fmadd_pd(w3, h0tmpv, sum3);
            sum4 = _mm256_fmadd_pd(w4, h0tmpv, sum4);

        }

        _mm256_store_pd(arrsum1, sum1);
        _mm256_store_pd(arrsum2, sum2);
        _mm256_store_pd(arrsum3, sum3);
        _mm256_store_pd(arrsum4, sum4);
        ...
        p9 = sigmoid(arrsum3[0]);
        p10= sigmoid(arrsum3[1]);
        p11= sigmoid(arrsum3[2]);
        p12= sigmoid(arrsum3[3]);
        p13= sigmoid(arrsum4[0]);
        p14= sigmoid(arrsum4[1]);
        p15= sigmoid(arrsum4[2]);
        p16= sigmoid(arrsum4[3]);
        ...
        k = i + 8;
        if (rand_n1 < p9) x[k] = 1; else x[k] = 0;
        if (rand_n2 < p10) x[k+1] = 1; else x[k+1] = 0;
        if (rand_n3 < p11) x[k+2] = 1; else x[k+2] = 0;
        if (rand_n4 < p12) x[k+3] = 1; else x[k+3] = 0;
        if (rand_n5 < p13) x[k+4] = 1; else x[k+4] = 0;
        if (rand_n6 < p14) x[k+5] = 1; else x[k+5] = 0;
        if (rand_n7 < p15) x[k+6] = 1; else x[k+6] = 0;
        if (rand_n8 < p16) x[k+7] = 1; else x[k+7] = 0;
    }
