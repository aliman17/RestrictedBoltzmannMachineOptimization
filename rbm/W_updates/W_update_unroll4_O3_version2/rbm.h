...
void W_update(double * W, double * h0_cap, double * h1_cap,
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < D; j+=4)
        {
            W[iD+j] = h0scalar_lambda * x0[j] - (h1scalar_lambda * x1[j] - W[iD+j]);
            W[iD+j+1] = h0scalar_lambda * x0[j+1] - (h1scalar_lambda * x1[j+1] - W[iD+j+1]);
            W[iD+j+2] = h0scalar_lambda * x0[j+2] - (h1scalar_lambda * x1[j+2] - W[iD+j+2]);
            W[iD+j+3] = h0scalar_lambda * x0[j+3] - (h1scalar_lambda * x1[j+3] - W[iD+j+3]);
        }
    }
}
