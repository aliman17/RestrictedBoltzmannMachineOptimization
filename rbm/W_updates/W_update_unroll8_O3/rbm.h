...
void W_update(double * W, double * h0_cap, double * h1_cap,
    ...
    // Scalar replacement
    // Loop unrolling: by 8
    double WiD, WiD1, WiD2, WiD3, WiD4, WiD5, WiD6, WiD7;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < D; j+=8)
        {
            W[iD + j] = W[iD + j] + lambda * (h0scalar * x0[j] - h1scalar * x1[j]);
            W[iD + j+1] = W[iD + j+1] + lambda * (h0scalar * x0[j+1] - h1scalar * x1[j+1]);
            W[iD + j+2] = W[iD + j+2] + lambda * (h0scalar * x0[j+2] - h1scalar * x1[j+2]);
            W[iD + j+3] = W[iD + j+3] + lambda * (h0scalar * x0[j+3] - h1scalar * x1[j+3]);
            W[iD + j+4] = W[iD + j+4] + lambda * (h0scalar * x0[j+4] - h1scalar * x1[j+4]);
            W[iD + j+5] = W[iD + j+5] + lambda * (h0scalar * x0[j+5] - h1scalar * x1[j+5]);
            W[iD + j+6] = W[iD + j+6] + lambda * (h0scalar * x0[j+6] - h1scalar * x1[j+6]);
            W[iD + j+7] = W[iD + j+7] + lambda * (h0scalar * x0[j+7] - h1scalar * x1[j+7]);
        }
        // We don't need additional patch, because D mod 2 = 0
    }
}
