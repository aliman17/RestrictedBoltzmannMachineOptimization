...
void h1_cap_update_W_update(double * h, int y0, double * x0, double *x1) {
    ...
    for (int i = 0; i < n ; i++)
    {
        ...
        if (i >= 1) {
            h0scalar_lambda = lambda * h0_cap[i - 1];
            h1scalar_lambda = lambda * h1_cap[i - 1];
            h0sc = _mm_load_sd(&h0scalar_lambda);
            h1sc = _mm_load_sd(&h1scalar_lambda);
        }
        ...
        for (int j = 0; j < D; j += 4)
        {
            ...
            // Update W[j-1]
            if (i >= 1) {
                w1 = _mm_load_sd(W + iD_1 + j);
                w2 = _mm_load_sd(W + iD_1 + j + 1);
                w3 = _mm_load_sd(W + iD_1 + j + 2);
                w4 = _mm_load_sd(W + iD_1 + j + 3);
                ...
                _mm_store_sd(W + iD_1 + j, tmp21);
                _mm_store_sd(W + iD_1 + j + 1, tmp22);
                _mm_store_sd(W + iD_1 + j + 2, tmp23);
                _mm_store_sd(W + iD_1 + j + 3, tmp24);
