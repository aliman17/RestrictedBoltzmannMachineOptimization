...
# Set basic plot construction, that we can just append different lines later
get_base_plot <- function(name, y_lim){
    p <- qplot(
        ...
        annotate("text", label = "scalar peak performance", x = 1600, y = 4.13, size = 5, colour = "#dd0000") + 
...
# Create function which adds aditional line on to the plot
plot_compare <- function(SELECTED_FUNCTION_NAME, NAMES, POSITION, COLORS, DIRS, y_lim){
    ...
    while (counter > 0){
        setwd(MAIN_DIR) # reset directory if multiple optimization line plotting (not needed, but in case)
        dir = DIRS[counter]
        print(dir)
        setwd(dir)
        ...
        for(i in 1:nrow(config)){
            ...
            if (name == SELECTED_FUNCTION_NAME){
                ...
                p <- p + geom_area(data = data, aes(x=x, y=y), col = col, alpha = 0.2, size=1) + 
                    annotate("text", label = NAMES[counter], x = 1000, y = POSITION[counter], size = 6, colour = col)
            }
        }
        counter <- counter - 1
    }
    setwd(MAIN_DIR) # reset directory back
