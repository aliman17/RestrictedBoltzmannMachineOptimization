...
#define n 50        //Number of hidden layers
...
{
    return 1.0 /(double) (1.0 + exp(-1.0 * val));
}
    ...
    for (i = 0; i < num_img_train; i++)
    {
        ...
        COD_training_update(get_label(i, labels_train), get_image(i, images_train));
         if(i%100==0)
         {
            ...
            predict_images ();
            printf(" Energy:%lf\n %lf", energy_for_all(), score_function());
         }
    }
}
...
int Predict_one_image(int *x) {
    ...
    for (int y = 0; y < K; y++) {
        ...
        numerators[y] = prod;
        //printf("Numerators: %lf ",numerators[y]);
