...
void h1_cap_update_W_update(double * h, int y0, double * x0, double * x1){
    
    __m256d  h0sc, h1sc;
    __m256d x1_, x2, x3, x4;
    __m256d w1, w2, w3, w4, w1_, w2_, w3_, w4_;
    ...
    for (int i = 0; i < n ; i++)
    {
        ...
        h0sc = _mm256_broadcast_sd(&h0scalar_lambda);
        h1sc = _mm256_broadcast_sd(&h1scalar_lambda);
        for (int j = 0; j < D; j+=16)
        {
            w1 = _mm256_loadu_pd(W + iD + j);
            w2 = _mm256_loadu_pd(W + iD + j + 4);
            ...
            x1_ = _mm256_loadu_pd(x0 + j );
            x2 = _mm256_loadu_pd(x0 + j + 4);
            x3 = _mm256_loadu_pd(x0 + j + 8);
            x4 = _mm256_loadu_pd(x0 + j + 12);
            
            s1 = _mm256_fmadd_pd(w1, x1_, s1);
            s2 = _mm256_fmadd_pd(w2, x2, s2);
            s3 = _mm256_fmadd_pd(w3, x3, s3);
            s4 = _mm256_fmadd_pd(w4, x4, s4);
            ...
            if (i>=1){
                ...
                w1_ = _mm256_loadu_pd(W + iD_1 + j);
                w2_ = _mm256_loadu_pd(W + iD_1 + j + 4);
                w3_ = _mm256_loadu_pd(W + iD_1 + j + 8);
                w4_ = _mm256_loadu_pd(W + iD_1 + j + 12);
                ...
                tmp11 = _mm256_fmsub_pd(h1sc, x11d, w1_);
                tmp12 = _mm256_fmsub_pd(h1sc, x12d, w2_);
                tmp13 = _mm256_fmsub_pd(h1sc, x13d, w3_);
                tmp14 = _mm256_fmsub_pd(h1sc, x14d, w4_);
                ...
                // Original: W[iD_1+j] = W[iD_1+j] + (h0scalar_lambda * x0 - h1scalar_lambda * x1);
                // Rearranged: W[iD_1+j] = h0scalar_lambda * x0 - (h1scalar_lambda * x1 - W[iD_1+j]);
                
                _mm256_store_pd(W + iD_1 + j, tmp21);
                _mm256_store_pd(W + iD_1 + j + 4, tmp22);
                _mm256_store_pd(W + iD_1 + j + 8, tmp23);
                _mm256_store_pd(W + iD_1 + j + 12, tmp24);
            }
        }
                         ...
                         double * U, double * W, int n, int D, int K) //DONE
{
    ...
    // Update Phase
        ...
        start = start_tsc();
    #endif
        h1_cap_update_W_update(h1_cap, y1, img_tmp_mem0, img_tmp_mem1);
    #ifdef PERF_W_UPDATE
        myInt64 cycles = stop_tsc(start);
        count = count + 1;
        delta = cycles - mean;
