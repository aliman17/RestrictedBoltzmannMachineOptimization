...
get_base_plot <- function(name, y_lim){
  p <- qplot(
      main = "Training Phase Performance",
      ylim = y_lim,
  ...
  annotate("text", label = "scalar peak performance", x = 1600, y = 3.9, size = 5, colour = "#dd0000") + theme(text = element_text(size=20))
  return(p)
}
...
plot_compare <- function(SELECTED_FUNCTION_NAME, NAMES, POSITION, COLORS, DIRS, y_lim){
  ...
  p <- p + geom_vline(xintercept = 40, col = "black", linetype = "longdash") +
            ...
            annotate("text", label = "L2", x = 110, y = 3.8, size = 6, colour = "#555555") +
            annotate("text", label = "L3", x = 460, y = 3.8, size = 6, colour = "#555555")
...
NAMES = c("Baseline", "Scalar", "Vectorized", "Vect. + H_W_fused_update")
DIRS = c( "./baseline/",
          "./scalar/merged/",
          "./vector/merged/",
            "./vector/h_update_w_update")
POSITION = c(0.12, 0.9, 2.0, 2.6)
COLORS = c("#003300", "#dd0000", "#0000ff", "#00aa00")
